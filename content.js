...
class LinkedInProfileExtractor {
  ...
  /**
   * Extract bio/about section from profile
   */
  extractBio() {
    console.log('📄 Starting bio/about extraction...');
    
    const selectors = [
      // Most reliable: span with aria-hidden="true" in About section
      'section[data-section="about"] .pv-shared-text-with-see-more span[aria-hidden="true"]',
      'section[data-section="about"] .inline-show-more-text span[aria-hidden="true"]',
      'section[data-section="about"] span[aria-hidden="true"]',
      
      // About section with inline-show-more-text
      'section[data-section="about"] .inline-show-more-text',
      'section[data-section="about"] .pv-shared-text-with-see-more',
      
      // Legacy about section selectors
      ...
      '.about-section .pv-about__summary-text span[aria-hidden="true"]',
      '.about-section .inline-show-more-text span[aria-hidden="true"]',
      '.about-section .pv-about__summary-text .inline-show-more-text',
      '.about-section .pv-about__summary-text',
      
      // Generic about selectors
      '.pv-about span[aria-hidden="true"]',
      '.pv-about .inline-show-more-text',
      '.about span[aria-hidden="true"]',
      '.about .inline-show-more-text',
      
      // Profile section about
      '.pv-profile-section.about span[aria-hidden="true"]',
      '.pv-profile-section.about .pv-shared-text-with-see-more span[aria-hidden="true"]',
      '.pv-profile-section.about .inline-show-more-text',
      
      // Fallback selectors
      '[data-generated-suggestion-target="about"] span[aria-hidden="true"]',
      '[data-generated-suggestion-target="about"] .inline-show-more-text',
      '.artdeco-card.about span[aria-hidden="true"]'
    ];

    const element = this.findElement(selectors);
    let bio = element ? this.cleanText(element.textContent) : '';
    
    console.log('📝 Bio/About extraction (before cleaning):');
    console.log(`  Raw text: "${element?.textContent}"`);
    console.log(`  After cleanText: "${bio}"`);
    
    // Additional cleaning for bio text
    if (bio) {
      // Remove "Show more" or "See more" text that might be included
      bio = bio.replace(/\s*(Show more|See more|\.\.\.show more|\.\.\.see more|…see more)\s*$/i, '');
      // Remove leading/trailing ellipsis
      bio = bio.replace(/^(\.\.\.+|…+)\s*/, '').replace(/\s*(\.\.\.+|…+)$/, '');
    }
    
    console.log(`  After additional cleaning: "${bio}"`);
    console.log('Extracted bio:', bio ? bio.substring(0, 100) + '...' : 'No bio found');
    return bio;
  }
